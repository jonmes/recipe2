        ...
        <template v-else>
            <div class="max-w-screen-xl items-center py-6 px-6 mx-auto md:px-12 lg:px-16 xl:px-24">
                <div class="py-6 w-full">
                    ...
                    <div class="max-w-7xl mx-auto">
                        <div class="flex items-center space-x-2 text-gray-400 text-sm">
                            <router-link :to="{ name: 'Home' }" class="hover:underline hover:text-gray-600">Home
                            </router-link>
                            <span>
                            ...
                            <router-link :to="{
                                name: 'Browse',
                            }" class="hover:underline hover:text-gray-600">Browse</router-link>
                            <span>
                                <svg class="h-5 w-5 leading-none text-gray-300" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mb-10 grid grid-cols-6 items-center">
                    ...
                    />-->
                    <vue3starRatings class="stars" id="stars" v-model="recipe.avg_rating" starSize="25"
                        starColor="#10B981" inactiveColor="#e6ebdf" controlBg="transparent" :showControl="false"
                        :disableClick="true" controlSize="0" />
                    <div class="mr-10 inline-block">{{ recipe.ratings.length }} Ratings</div>
                    <div class="inline-block col-span-4">
                        <Popper arrow>
                            <button
                                class="bg-green sm:w-auto h-8 px-8 font-large text-white rounded-xl whitespace-nowrap hover:shadow-xl transition-shadow duration-300">Rate</button>
                            <template #content="{ close }">
                                <div class="flex justify-center w-full mb-7">
                                    <StarRate class="text-black" v-model="currentRating" :colours="{
                                        activeColour: 'darkgreen',
                                        hoverColour: '#32CD32',
                                        inactiveColour: '#EFF'
                                    }" />
                                ...
                                <button
                                    ...
                                    @click="addrateRecipe(); close();">Rate</button>
                                <button
                ...
                <div
                    ...
                    <div
                        ...
                        <transition name="slide" mode="out-in">
                            <img class="absolute inset-0 w-full h-128 object-cover shadow-lg rounded-lg"
                                :src="recipe.images[imgToShow].url" alt="recipeImage" />
                        </transition>
                        ...
                        <div
                            ...
                            <div v-for="(imageP, index) in recipe.images" :key="index" tabindex="0"
                                class="relative h-0 pb-8 sm:pt-1/3 lg:pt-24">
                                <img class="absolute inset-0 w-32 h-32 object-cover shadow-lg rounded-lg"
                                    :src="imageP.url" alt="recipeImage" @click="showImage(index)" />
                            </div>
                        </div>
                    </div>

                    <div class="relative h-0 mt-96 pt-10 sm:pt-0 sm:mt-48 md:mt-128 lg:mt-0 xl:mt-0 pb-0 md:mb-72">
                        <div class="px-4">
                            <h2
                                ...
                                <button type="button"
                                    ...
                                    <svg width="32px" height="32px" viewBox="0 0 32 32"
                                        ...
                                        <path
                                            ...
                                            fill="none" stroke="black" stroke-linejoin="round" stroke-miterlimit="10"
                                            stroke-width="2" />
                                    </svg>
                ...
                <div class="w-full mb-32 lg:mt-60 xl:mt-0">
                    ...
                    <ul v-for="(ing, index) in recipe.ingredients" :key="index" class="text-2xl ml-10 pb-3">
                        <li>
                    ...
                    <div class="mt-10">
                        ...
                        <ul v-for="(step, index) in recipe.steps" :key="index" class="text-2xl ml-10 pb-3">
                            <li>
                                <span>
                                    <img src="https://img.icons8.com/ios-glyphs/30/000000/left-footprint--v2.png"
                                        class="inline-block" />
                                    Step {{ index + 1 }}
                                </span>
                ...
                <form @submit="addComment">
                    <div>
                        <Field as="input" type="text" name="comment" rules="required"
                ...
                <div class="bg-white rounded-lg p-3 flex flex-col justify-center items-center md:items-start shadow-lg mb-4"
                    ...
                    <div class="flex flex-row justify-center mr-2">
                        ...
                        <h3 class="text-green-600 font-semibold text-lg text-center md:text-left">{{ comment.user.name
                        }}</h3>
                    </div>
...
import { query_all, rating_query, comment_query, fav_check, fav_count, fav_query } from '../graphql/query'
import { rating_mutation, comment_mutation, fav_mutation, delete_fav } from '../graphql/mutation'
...
const addFavorite = () => {
    if (localStorage.getItem('user')) {
        if (!liked.value) {
            ...
            deleteFav_updateCache(favId.value)
            liked.value = false
        }
    } else {
