    ...
    <div class="min-h-screen bg-gradient-to-br from-transparent to-green-100">
        <div class="max-w-screen-xl items-center py-6 px-6 mx-auto md:px-12 lg:px-16 xl:px-24">
            <div class="container mx-auto pt-12">
                ...
                <section>
                    <div class="mt-10 mb-10 w-full">
                        <ul class="w-full grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                            <li @click="catFilter = 'Fruits_and_Vegetables'; isCat = true"
                            ...
                            <li @click="catFilter = 'Breads_and_Sweets'; isCat = true"
                                class="m-3.5 h-52 w-full bg-gray-100 rounded-xl flex flex-col items-center justify-center text-center duration-300 hover:bg-white hover:shadow-2xl">
                ...
                <section class="mb-10">
                    <div class="relative flex p-1 rounded-full bg-white border border-yellow-200 shadow-md md:p-2 mb-7">
                        <input placeholder="Your favorite food"
                            ...
                            type="text" v-model="search" @keypress.enter="setSearch()" />
                        <button type="button" title="Start buying"
                            ...
                            <span class="inline-block">
                                <svg xmlns="http://www.w3.org/2000/svg" class="bi bi-search" fill="currentColor"
                                    height="20" width="20" viewBox="0 0 16 16">
                                    <path
                    ...
                    <div v-if="open == true" class="grid grid-cols-3 mb-5 mt-5">
                        <div class="space-y-2">
                            <h1>Duration</h1>
                            <hr class="w-40 mb-4" />
                            <button class="block hover:font-bold" :class="{ 'font-bold': time_lte == 15 }"
                                @click="filterTime(1); open = false; recipeRefetch()">Under 15 minutes</button>
                            <button class="block hover:font-bold" :class="{ 'font-bold': time_gt == 15 }"
                                @click="filterTime(2); open = false; recipeRefetch()">15 - 30 minutes</button>
                            <button class="block hover:font-bold" :class="{ 'font-bold': time_gt == 30 }"
                                @click="filterTime(3); open = false; recipeRefetch()">Over 30 minutes</button>
                            <button class="block hover:font-bold"
                                @click="time_gt = 0; time_lte = 10000; open = false; page = 0; recipeRefetch()">Cancel
                                Filter</button>
                        </div>
                        <div class="space-y-2">
                            <h1>Sorty by</h1>
                ...
                <section v-if="isCat">
                    ...
                    <template v-if="catE">Something went wrong... {{ catL }}</template>
                    <template v-if="catL">
                    ...
                    <template v-else>
                        <div class="w-full h-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-x-7 gap-y-4"
                            ref="scrollComponent">
                            ...
                            <div class="max-w-xs mb-5 rounded-md overflow-hidden hover:scale-101 hover:shadow-2xl transition duration-100 cursor-pointer"
                                ...
                                <router-link role="button" :to="{
                                    ...
                                    <div>
                                        <img class="w-80 h-80 bg-clip-content" :src="rec.images[0].url" alt="pic" />
                                    </div>
                                    <div class="py-4 px-4 bg-white h-full">
                                        ...
                                        <div class="flex items-center mt-2.5 mb-5">
                                            <vue3starRatings class="stars" id="stars" v-model="rec.avg_rating"
                                                starSize="25" starColor="#10B981" inactiveColor="#e6ebdf"
                                                controlBg="transparent" :showControl="false" :disableClick="true"
                                                controlSize="0" />
                                            <span
                ...
                <section v-if="!isCat">
                    ...
                    <template v-else>
                        <div class="w-full h-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-x-7 gap-y-4"
                            ...
                            <div class="max-w-xs mb-5 rounded-md overflow-hidden hover:scale-101 hover:shadow-2xl transition duration-100 cursor-pointer"
                                v-else v-for="(rec, index) in searchRecipe" :key="index">
                                <router-link role="button" :to="{
                                    name: 'Details',
                                    params: { id: rec.id },
                                    query: { name: rec.title, id: rec.id }
...
import { search_recipe, all_recipe, ctg_filter } from '../graphql/query'
